//
//  NationIMProjectViewController.m
//  hinabian
//
...
enum IMNationHome_Item_Index
{
    ...
    IM_NATION_PROJECT_TITLE,
    IM_NATION_EMITY_TWO,
    IM_NATION_VISA_TITLE,
};

@interface NationIMProjectViewController ()<UITableViewDelegate,UITableViewDataSource,ScrollBannerViewDelegate,RefreshControlDelegate>
...
@implementation NationIMProjectViewController
...
- (void)viewDidLoad {
    ...
    self.tableView = [[UITableView alloc] initWithFrame:CGRectMake(0, 0,SCREEN_WIDTH, SCREEN_HEIGHT - SCREEN_STATUSHEIGHT - SCREEN_TABHEIGHT - SCREEN_PROJ_NAVHEIGHT)];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    else if (IM_NATION_PROJECT_TITLE == indexPath.row) {
    ...
    /*项目数据*/
    else if (indexPath.row > IM_NATION_PROJECT_TITLE && indexPath.row < IM_NATION_VISA_TITLE + _projDatas.count) {
        ...
        NSInteger index = indexPath.row - IM_NATION_PROJECT_TITLE - 1;
        IMHomeProjModel *model = _projDatas[index];
        [cell setProjModel:model];
        returnCellPtr = cell;
    }
    else if (IM_NATION_VISA_TITLE + _projDatas.count == indexPath.row) {
    ...
    /*签证数据*/
    else if (indexPath.row > IM_NATION_VISA_TITLE + _projDatas.count) {
        ...
        NSInteger index = indexPath.row - _projDatas.count - IM_NATION_VISA_TITLE - 1;
...
//        return IM_NATION_VISA_TITLE + _projDatas.count + _visaDatas.count + 1;
//    } else {
//        return IM_NATION_PROJECT_TITLE + _projDatas.count + _visaDatas.count + 1;
//    }
    NSUInteger tempNum = _visaDatas.count > 0 ? IM_NATION_VISA_TITLE + _projDatas.count + _visaDatas.count + 1 : IM_NATION_PROJECT_TITLE + _projDatas.count + _visaDatas.count + 1;
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.row == IM_NATION_SHUFFLING_IMAGE) {
        ...
        return 120.f;
    }else if (indexPath.row == IM_NATION_EMITY_ONE || indexPath.row == IM_NATION_EMITY_TWO + _projDatas.count) {
        return 10.f;
    }else if (indexPath.row == IM_NATION_PROJECT_TITLE || IM_NATION_VISA_TITLE + _projDatas.count == indexPath.row) {
...
//    NSLog(@" \n \n %s \n \n ",__FUNCTION__);
    if (indexPath.row > IM_NATION_PROJECT_TITLE && indexPath.row < IM_NATION_VISA_TITLE + _projDatas.count) {
        NSInteger index = indexPath.row - IM_NATION_PROJECT_TITLE - 1;
        IMHomeProjModel *model = _projDatas[index];
        /*数据上报*/
