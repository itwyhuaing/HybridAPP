...
@interface IMProjectHomeViewController ()<TYTabPagerBarDelegate,TYTabPagerControllerDataSource,TYTabPagerControllerDelegate,RefreshControlDelegate,HNBNetRemindViewDelegate>
{
    BOOL isLeaveTab;
    BOOL isLeavePro;
}
...
@implementation IMProjectHomeViewController
...
//        [self backHandleContryAndProjectData:JSON];
//        dispatch_group_leave(indexLoad);
//    } withFailHandler:^(id error) {
//        dispatch_group_leave(indexLoad);
//    }];
    ...
    [DataFetcher gettabInIMProjectHomeWithLocalCachedHandler:^(id JSON) {
        [self backHandleTabData:JSON];
        isLeaveTab = [self queryIsLeaveGrop:isLeaveTab queueGroup:indexLoad];
    ...
    [DataFetcher doGetContryAndProjectWithLocalCachedHandler:^(id JSON) {
        [self backHandleContryAndProjectData:JSON];
        isLeavePro = [self queryIsLeaveGrop:isLeavePro queueGroup:indexLoad];
    } succeedHandler:^(id JSON) {
        [self backHandleContryAndProjectData:JSON];
        isLeavePro = [self queryIsLeaveGrop:isLeavePro queueGroup:indexLoad];
    } withFailHandler:^(id error) {
...
- (void)backHandleTabData:(id)JSON{
...
- (void)backHandleContryAndProjectData:(id)JSON{
    if ([JSON isKindOfClass:[NSArray class]]) {
        NSArray *arr = JSON;
...
- (UIViewController *)tabPagerController:(TYTabPagerController *)tabPagerController controllerForIndex:(NSInteger)index prefetching:(BOOL)prefetching {
        if (index == 0) {
            //热门项目页
            //HotIMProjectViewController *VC = [[HotIMProjectViewController alloc]init];
            HotIMProjectVC *VC = [[HotIMProjectVC alloc]init];
            ...
            //国家项目页
            NationIMProjectViewController *VC = [[NationIMProjectViewController alloc]init];
            IMHomeNationTabModel *model = _datas[index];
            VC.f_id = model.f_id;
            VC.f_title = model.f_name_short_cn;
