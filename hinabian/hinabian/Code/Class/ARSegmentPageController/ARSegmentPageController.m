...
- (void)_baseConfigs {
  ...
  [self.controllers
    ...
    if(segmentDefaultIndex >= self.controllers.count)
    {
        segmentDefaultIndex = 0;
    }

    self.segmentView.segmentControl.selectedSegmentIndex = segmentDefaultIndex;
    UIViewController<ARSegmentControllerDelegate> *controller = self.controllers[segmentDefaultIndex];
  [controller willMoveToParentViewController:self];
  [self.view insertSubview:controller.view atIndex:segmentDefaultIndex];
  ...
  self.currentDisplayController = self.controllers[segmentDefaultIndex];
