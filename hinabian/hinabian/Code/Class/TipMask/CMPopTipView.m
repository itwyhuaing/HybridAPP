...
- (void)drawRect:(__unused CGRect)rect
{
	...
	if (_pointDirection == PointDirectionUp) {
        ...
        CGContextSetRGBStrokeColor(context, 1, 1, 1, 1);
            ...
            CGContextAddArc(context, _targetPoint.x, _targetPoint.y-wan+dis, radius, 90*(M_PI/180)+(dis/2/radius), 270*(M_PI/180)-(dis/2/radius), 1);
            CGContextStrokePath(context);
        
        CGContextAddArc(context, _targetPoint.x, _targetPoint.y-wan, radius, 90*(M_PI/180)+(dis/2/radius), 270*(M_PI/180), 1);
        CGContextStrokePath(context);
        
        CGContextAddArc(context, _targetPoint.x, _targetPoint.y-wan+dis*2, radius, 90*(M_PI/180), 270*(M_PI/180)-(dis/2/radius), 1);
        CGContextStrokePath(context);
        ...
        CGContextSetRGBStrokeColor(context, 1, 1, 1, 1);
        for (int i = 0; i<num; i++) {
            CGContextAddArc(context, _targetPoint.x, _targetPoint.y-wan+dis*i, radius, 90*(M_PI/180)+(dis/2/radius), 270*(M_PI/180)-(dis/2/radius), 1);
            CGContextStrokePath(context);
        }
        CGContextAddArc(context, _targetPoint.x, _targetPoint.y-wan, radius, 90*(M_PI/180)+(dis/2/radius), 270*(M_PI/180), 1);
        CGContextStrokePath(context);
...
#pragma clang diagnostic pop
	...
	CGFloat fullHeight = _bubbleSize.height + _pointerSize + 20.0;
	CGFloat y_b;
	if (_pointDirection == PointDirectionUp) {
...
- (void)presentPointingAtBarButtonItem:(UIBarButtonItem *)barButtonItem animated:(BOOL)animated {
	_targetView = (UIView *)[barButtonItem performSelector:@selector(view)];
	...
	[self presentPointingAtView:_targetView inView:containerView animated:animated];
}
...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event {
    // Rain 修改 - 将点击事件传递出去
    if ([_delegate respondsToSelector:@selector(popTipViewWasDismissedByUser:)]) {
        [_delegate popTipViewWasDismissedByUser:self];
    } else {
        [self dismissByUser];
    }
