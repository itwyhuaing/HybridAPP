...
-(void)POST:(NSString *)URLString parameters:(id)parameters success:(HNBAFNetToolSuccess)success failure:(HNBAFNetToolFailure)failure{
    ...
    NSTimeInterval inTime=[dat timeIntervalSince1970];
    NSString *netStatus = [HNBUtils returnCurrentEquipmentNetStatus];
    ...
    [afnManager POST:URLString parameters:parameters progress:^(NSProgress * _Nonnull uploadProgress) {
        ...
        NSDate* dat = [NSDate dateWithTimeIntervalSinceNow:0];
        NSTimeInterval outTime=[dat timeIntervalSince1970];
        //[HNBInterfaceTime recordInterfaceTime:URLString IsSuccessed:TRUE Time:(outTime-inTime)];
        [HNBInterfaceTime reportInterfaceTime:URLString IsSuccessed:TRUE Time:(outTime-inTime) netStatus:netStatus];
        ...
        if (error.code == -1009) {
            ...
            //如果不是无网络而请求失败，上报错误
            [HNBUtils upLoadErrortype:@"NetWork" API:URLString netError:error];
