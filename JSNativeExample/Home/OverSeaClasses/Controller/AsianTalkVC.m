...
@interface AsianTalkVC ()<UITableViewDelegate,UITableViewDataSource,RefreshControlDelegate,HNBClassesCellDelegate,OverSeaClassesManagerDelegate>
...
- (void)viewDidLoad {
    ...
    backgroundImageView.image = [UIImage imageNamed:@"OverSea_AsianTalkBG"];
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    [HNBClick event:@"206011" Content:nil];
    
    if ([HNBUtils isLogin]) {
        [DataFetcher doGetPersonalIsFollow:overseaclass_xiaona_id WithSucceedHandler:^(id JSON) {
...
//    [DataFetcher doGetListOverSeaClassWithType:overseaclass_asiantalk page:[NSString stringWithFormat:@"%ld",_currentPage] num:PageNum WithSucceedHandler:^(id JSON) {
...
//    }];
    ...
    [self.manager reqNetDataWithType:overseaclass_asiantalk
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.row >= 1) {
        ...
        [HNBClick event:@"206031" Content:dict];
    }
}
...
- (void)clickCareBtn
{
    ...
    NSString *tmpState = @"1";
    if (_isFollow) {
        [DataFetcher removeFollowWithParameter:overseaclass_xiaona_id withSucceedHandler:^(id JSON) {
        ...
        [DataFetcher addFollowWithParameter:overseaclass_xiaona_id withSucceedHandler:^(id JSON) {
            _careLabel.text = CareText;
            _isFollow = YES;
        } withFailHandler:^(id error) {
            _careLabel.text = UnCareText;
