...
@interface IMClassesVC ()<UITableViewDelegate,UITableViewDataSource,RefreshControlDelegate,OverSeaClassesManagerDelegate>
...
- (void)viewDidLoad {
    ...
    backgroundImageView.image = [UIImage imageNamed:@"OverSea_IMClassBG"];
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    [HNBClick event:@"206012" Content:nil];
    
    if ([HNBUtils isLogin]) {
        [DataFetcher doGetPersonalIsFollow:overseaclass_xiaobian_id WithSucceedHandler:^(id JSON) {
...
//    [DataFetcher doGetListOverSeaClassWithType:overseaclass_imm page:@"1" num:PageNum WithSucceedHandler:^(id JSON) {
...
//    }];
    ...
    [self.manager reqNetDataWithType:overseaclass_imm
                                page:@"1"
                                 num:PageNum];
...
- (void)refreshData
{
//    [DataFetcher doGetListOverSeaClassWithType:overseaclass_imm page:[NSString stringWithFormat:@"%ld",_currentPage] num:PageNum WithSucceedHandler:^(id JSON) {
...
- (void)clickCareBtn
{
    ...
    NSString *tmpState = @"1";
    if (_isFollow) {
        [DataFetcher removeFollowWithParameter:overseaclass_xiaobian_id withSucceedHandler:^(id JSON) {
        ...
        [DataFetcher addFollowWithParameter:overseaclass_xiaobian_id withSucceedHandler:^(id JSON) {
            _careLabel.text = CareText;
            _isFollow = YES;
        } withFailHandler:^(id error) {
            _careLabel.text = UnCareText;
